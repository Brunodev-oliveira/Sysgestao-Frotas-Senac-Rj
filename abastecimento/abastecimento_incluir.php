		...
		<div id="contents"> 
			...
			// Abre o arquivo com as conexões do BD
				...
				if(isset($_POST["incluir"])) {
					...
					$query = mysql_query("INSERT INTO abastecimento VALUES('','$data','$hora','$local','$valor','$placa','$matricula')", $cnn) or die(mysql_error()); 
					$result = mysql_affected_rows();
					if($result > 0){
						echo "<script> alert('Abastecimento Cadastrado!'); </script>";
						echo "<script> window.location='abastecimento.php';</script>";
					}
					else{
						echo "<script> alert('Abastecimento não Cadastrado!'); </script>";
						echo "<script> window.location='abastecimento.php';</script>";
					}
				}
