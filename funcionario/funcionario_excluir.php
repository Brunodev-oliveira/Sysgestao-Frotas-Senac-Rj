		...
		<div id="contents"> 
			...
			// Abre o arquivo com as conexões do BD
				...
				if(isset($_POST["excluir"])) {	
					...
					$query = mysql_query("DELETE FROM funcionario WHERE matricula = '$matricula'", $cnn) or die(mysql_error());      
					$result = mysql_affected_rows();
					if($result > 0){
						echo "<script> alert('Funcionário Excluido!'); </script>";
						echo "<script> window.location='funcionario.php';</script>";
					}
					else{
						echo "<script> alert('Funcionário não Excluido!'); </script>";
						echo "<script> window.location='funcionario.php';</script>";
					}
				}
